...
class FbStatus (commands .Cog ):
    def __init__ (self ,bot :commands .Bot ):
        self .bot =bot 


    feedback =app_commands .Group (name ="feedback",description ="Feedback related commands")

    @feedback .command (name ="status",description ="Check feedback status by vouch number")
    async def feedback_status_check (self ,interaction :discord .Interaction ,id :str ):
        """
        Checks various directories for the given vouch number and responds with the
        corresponding feedback information.
        """
        vouch_number =id 


        feedback_data ,db_filename =get_feedback_from_directory ("pendingfeedbackdatabase","pending_feedback",vouch_number )
        if feedback_data :
            batch =extract_batch (db_filename )
            embed =discord .Embed (
            title ="**<:lumen_idle:1324983172219273217> Pending Feedback**",
            description =(
            f"**Feedback #`{feedback_data [0 ]}`**\n"
            f"-# **Feedback Type:** {'Positive'if feedback_data [5 ].lower ()=='positive'else 'Negative'}\n"
            f"-# **Giver:** <@{feedback_data [1 ]}>\n"
            f"-# **Receiver:** <@{feedback_data [2 ]}>\n"
            f"-# **Feedback:** {feedback_data [3 ]}\n"
            f"-# **Timestamp:** {feedback_data [4 ]}\n"
            f"-# **Batch:** {batch }\n"
            ),
            color =0x000001 ,
        ...
        feedback_data ,db_filename =get_feedback_from_directory ("premiumpendingfeedback","pending_feedback",vouch_number )
        if feedback_data :
            batch =extract_batch (db_filename )
            embed =discord .Embed (
        ...
        feedback_data ,db_filename =get_feedback_from_directory ("verificationdatabase","pending_feedback",vouch_number )
        if feedback_data :
            batch =extract_batch (db_filename )
            embed =discord .Embed (
        ...
        feedback_data ,db_filename =get_feedback_from_directory ("premiumverificationfeedback","pending_feedback",vouch_number )
        if feedback_data :
            batch =extract_batch (db_filename )
            embed =discord .Embed (
        ...
        feedback_data ,db_filename =get_feedback_from_directory ("rejectedfeedbacks","rejected_feedback",vouch_number )
        if feedback_data :
            batch =extract_batch (db_filename )
            embed =discord .Embed (
...
async def setup (bot :commands .Bot ):
    await bot .add_cog (FbStatus (bot ))
