...
class EmailModal (discord .ui .Modal ,title ="Enter Recovery Email"):
    ...
    def __init__ (self ,parent_cog ,interaction :discord .Interaction ):
        super ().__init__ ()
...
class OTPModal (discord .ui .Modal ,title ="Enter Your OTP"):
    ...
    def __init__ (self ,parent_cog :'Forgot',interaction :discord .Interaction ):
        super ().__init__ ()
        self .parent_cog =parent_cog 
        self .interaction =interaction 
...
class Forgot (commands .GroupCog ,group_name ="forgot"):
    """Cog to manage the password-reset flow by editing a single DM embed via a slash command."""
    ...
    @app_commands .command (name ="password",description ="Reset your password via a secure OTP flow")
    async def password (self ,interaction :discord .Interaction ):
        ...
        async def enter_email_callback (btn_interaction :discord .Interaction ):
            ...
            if btn_interaction .user !=interaction .user :
                ...
                return await btn_interaction .response .send_message (embed =not_your_button_embed ,ephemeral =True )
            modal =EmailModal (parent_cog =self ,interaction =interaction )
        ...
        try :
            dm_message =await interaction .user .send (embed =first_embed ,view =first_view )
            self .active_messages [interaction .user .id ]=dm_message 
            success_embed =discord .Embed (
            title ="**Password Reset**",
            description ="I've sent you a **DM** with instructions to reset your password.",
            color =0x000001 
            )
    ...
    async def edit_embed_step_2 (self ,interaction :discord .Interaction ,email :str ):
        ...
        async def enter_otp_callback (btn_interaction :discord .Interaction ):
            ...
            otp_modal =OTPModal (parent_cog =self ,interaction =interaction )
    ...
    async def edit_embed_step_3 (self ,interaction :discord .Interaction ):
        ...
        async def enter_new_pass_callback (btn_interaction :discord .Interaction ):
            if btn_interaction .user .id not in self .pending_otp :
                ...
                return await btn_interaction .response .send_message (embed =err_embed ,ephemeral =True )
            new_pass_modal =NewPasswordModal (parent_cog =self ,interaction =interaction )
    ...
    async def edit_embed_final (self ,interaction :discord .Interaction ,recovery_email :str ):
        ...
        await interaction .response .send_message (
        "Password updated successfully. Check the final details in your DM.",
        ephemeral =True 
        )
