...
class NegativeFeedbackCog (commands .GroupCog ,name ="negative"):
    def __init__ (self ,bot ):
        self .bot =bot 

    @app_commands .command (name ="feedback",description ="Submit negative feedback")
    ...
    async def feedback (self ,interaction :discord .Interaction ,user :discord .User ,feedback :str ):
        ...
        if not can_vouch_again (interaction .user .id ):
            embed =discord .Embed (
            title ="**Error**",
        ...
        try :
            ...
            description =(
            f"<:lumen_dnd:1324983165554524252> You have received a negative feedback from `{interaction .user .name }`.\n"
        ...
        async def send_feedback_to_channel ():
            if is_user_premium (user .id ):
                pending_channel =interaction .guild .get_channel (PREMIUM_PENDING_FEEDBACK_CHANNEL_ID )or await self .bot .fetch_channel (PREMIUM_PENDING_FEEDBACK_CHANNEL_ID )
                ...
                pending_channel =interaction .guild .get_channel (PENDING_FEEDBACK_CHANNEL_ID )or await self .bot .fetch_channel (PENDING_FEEDBACK_CHANNEL_ID )
                if not pending_channel :
...
async def setup (bot ):
    await bot .add_cog (NegativeFeedbackCog (bot ))
