...
class SetRecovery (commands .Cog ):
    def __init__ (self ,bot :commands .Bot ):
        self .bot =bot 

    async def recovery_email (self ,interaction :discord .Interaction ):
...
class OTPRequestView (ui .View ):
    ...
    async def enter_otp_button (self ,interaction :discord .Interaction ,button :ui .Button ):
        if not interaction .response .is_done ():
            await interaction .response .send_modal (OTPModal (self .generated_otp ,self .recovery_email ))
...
class ConfirmRecoveryView (ui .View ):
    ...
    async def save_to_database (self ,interaction :discord .Interaction ):
        ...
        try :
            ...
            cursor .execute ("SELECT 1 FROM users WHERE user_id = ?",(self .user .id ,))
...
async def recovery_email_callback (interaction :discord .Interaction ):
    cog =interaction .client .get_cog ("SetRecovery")
    if cog is None :
        await interaction .response .send_message ("Cog not found.",ephemeral =True )
        return 
    await cog .recovery_email (interaction )


recovery_email_command =app_commands .Command (
name ="email",
description ="Initiates the recovery email setup process via DM",
callback =recovery_email_callback 
)

set_group =app_commands .Group (name ="set",description ="Settings commands")
recovery_group =app_commands .Group (name ="recovery",description ="Recovery settings commands")
recovery_group .add_command (recovery_email_command )
set_group .add_command (recovery_group )

async def setup (bot :commands .Bot ):
    await bot .add_cog (SetRecovery (bot ))
