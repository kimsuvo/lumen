...
class PreHotLeaderboard (commands .Cog ):
    ...
    @commands .command ()
    async def hot (self ,ctx ,period :str =None ):
        """
        Usage: !hot <daily|weekly|monthly>
        Shows the top 5 users with the most positive feedback between:
            - Daily: 00:00 AM IST of today to now
            - Weekly: Monday 00:00 AM IST of the current week to now
            - Monthly: 1st day of the current month (00:00 AM IST) to now
        """
        ...
        description ="Please wait while we gather the hot users' feedback.",
        color =0x000001 
        )
...
async def setup (bot ):
    await bot .add_cog (PreHotLeaderboard (bot ))
