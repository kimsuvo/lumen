...
class PreSetRecovery (commands .Cog ):
    def __init__ (self ,bot ):
        self .bot =bot 

    @commands .command (name ="setrecovery")
    async def set_recovery (self ,ctx ):
        """Initiates the recovery email setup process via DM."""

        if not is_user_premium (ctx .author .id ):
            error_embed =discord .Embed (
            title ="**Upgrade to Lumen Premium**",
            description ="Recovery Email is a premium feature."
            " To access this feature, please consider upgrading to premium.",
            color =0xff0000 ,
            )
        ...
        db_path =find_user_database (ctx .author .id ,db_directory )
        if not db_path :
            error_embed =discord .Embed (
        ...
        try :
            ...
            view =EnterRecoveryView (ctx .author )
            await ctx .author .send (embed =embed ,view =view )
            confirmation_embed =discord .Embed (
            title ="**Recovery Email Setup Notification**",
            description =(
            f"{ctx .author .mention }, a message has been sent to your **Direct Messages (DMs)** to continue the recovery email setup.\n\n"
            "**__Next Steps:__**\n"
...
async def setup (bot ):
    await bot .add_cog (PreSetRecovery (bot ))
