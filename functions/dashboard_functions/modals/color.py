...
import discord 
from functions .dashboard_functions .profilesettings .color import update_dashboard_color ,fetch_dashboard_color 
...
class ColorModal (discord .ui .Modal ,title ="Update Color"):
    ...
    required =False ,
    max_length =7 ,
    placeholder ="e.g., #ff0000 or red (leave empty to clear)"
    )
    ...
    async def on_submit (self ,interaction :discord .Interaction ):
        new_color =self .color_input .value 
        try :
            updated_color =update_dashboard_color (self .user_id ,new_color )
